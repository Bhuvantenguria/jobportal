...
const companyLogoSchema = z.custom<File | undefined>()
    .refine((file) => file == null || (file instanceof File && file.type.startsWith("image/")), "Must be an image file")
    .refine((file) => !file || file.size < 1024 * 1024 * 0.5, "File must be less than 0.5MB");
